...
const canPay = computed(() => {
  ...
  // 优先使用路由参数中的healthcardId
  const cardId = route.query.healthcardId || searchForm.value.healthcardId
  ...
  try {
    ...
    const response = await queryPatients(cardId)
    ...
    // 更新表单中的就诊卡号显示
    searchForm.value.healthcardId = data.healthcardId || data.healthcard_id
...
onMounted(() => {
  ...
  if (route.query.healthcardId) {
    // 设置表单值但不立即触发查询
    searchForm.value.healthcardId = Number(route.query.healthcardId)
    // 延迟执行查询，确保DOM已更新
    nextTick(() => {
      loadPatientInfo()
    })
  ...
  try {
    ...
    ElMessage.error('创建挂号记录失败: ' + (error.message || '未知错误'))
  } finally {
